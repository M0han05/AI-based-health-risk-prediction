...
export default function LearningComparison({ patient, trainedAccuracy }: Props) {
  ...
  useEffect(() => {
    const fetchData = async () => {
      let currentAcc = trainedAccuracy;

      // If no prop provided, fetch the real-time global accuracy from backend
      if (!currentAcc) {
        try {
          const res = await fetch('http://localhost:5000/api/fl/global-model');
          const data = await res.json();
          currentAcc = data.accuracy;
        } catch {
          currentAcc = 68.4; // Fallback
        }
      }

      const r = await generateAccuracyResults(patient, currentAcc);
      setResults(r);
                  ...
                  {i === 0 && (
                    <div className="flex flex-col items-end gap-1">
                      <span className="text-[10px] bg-blue-500/20 text-blue-400 px-2 py-0.5 rounded-full font-bold uppercase tracking-tighter border border-blue-500/30">
                        {trainedAccuracy ? '✓ Live Trained' : 'Simulated'}
                      </span>
                      <span className="text-[9px] text-gray-500 uppercase">DP + Secure Aggregation</span>
                    </div>
                  )}
