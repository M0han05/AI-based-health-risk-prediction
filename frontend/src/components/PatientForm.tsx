...
export default function PatientForm({ onSubmit, initialData, onChange }: Props) {
  const [patient, setPatient] = useState<PatientData>(initialData || defaultPatient);
  const [step, setStep] = useState(0);

  const validateStep = (currentStep: number): boolean => {
    if (currentStep === 0) {
      if (!patient.aadhaarNumber || patient.aadhaarNumber.length < 12) return false;
      if (!patient.name || patient.name.trim().length < 2) return false;
    }
    if (currentStep === 1) {
      if (patient.bloodPressureSystolic <= 0) return false;
      if (patient.bloodPressureDiastolic <= 0) return false;
      if (patient.bloodSugar <= 0) return false;
      if (patient.cholesterol <= 0) return false;
      if (patient.bmi <= 0) return false;
      if (patient.heartRate <= 0) return false;
    }
    return true;
  };

  const validateStepWithAlert = (currentStep: number): boolean => {
    if (currentStep === 0) {
      if (!patient.aadhaarNumber || patient.aadhaarNumber.length < 12) {
    ...
    if (currentStep === 1) {
      if (patient.bloodPressureSystolic <= 0 || patient.bloodPressureDiastolic <= 0) {
        alert('Please enter vital Blood Pressure readings.');
        return false;
      }
      if (patient.bloodSugar <= 0) {
        alert('Please enter valid Blood Sugar level.');
        return false;
      }
      if (patient.cholesterol <= 0) {
        alert('Please enter valid Cholesterol level.');
        return false;
      }
      if (patient.bmi <= 0) {
        alert('Please enter valid BMI measurement.');
        return false;
      }
      if (patient.heartRate <= 0) {
        alert('Please enter valid Heart Rate (BPM).');
        return false;
      }
    }
    return true;
  ...
  const handleTabClick = (targetStep: number) => {
    if (targetStep > step) {
      for (let i = step; i < targetStep; i++) {
        if (!validateStepWithAlert(i)) return;
      }
    }
    setStep(targetStep);
  };
      ...
      <div className="flex justify-center gap-2 mb-8">
        {sections.map((s, i) => (
          <button
            ...
            disabled={i > step && !validateStep(step)}
          ...
          <div className="flex justify-between mt-8 pt-6 border-t border-gray-700/50">
            ...
            {step < 2 ? (
              <button
                type="button"
                onClick={() => {
                   if (validateStepWithAlert(step)) setStep(step + 1);
                }}
